...
import type { ArktypeNullable, ArktypeOptional, GetArktypeType, HandleColumn } from "./column.types.cjs";
...
export type BuildRefine<TColumns extends Record<string, any>> = {
    [K in keyof TColumns as TColumns[K] extends Column | SelectedFieldsFlat<Column> | Table | View ? K : never]?: TColumns[K] extends Column ? BuildRefineField<GetArktypeType<TColumns[K]>> : BuildRefine<GetSelection<TColumns[K]>>;
};
...
export type BuildSchema<TType extends 'select' | 'insert' | 'update', TColumns extends Record<string, any>, TRefinements extends Record<string, any> | undefined> = type.instantiate<Simplify<{
