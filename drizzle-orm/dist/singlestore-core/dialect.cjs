...
var import_temp_table = require("./temp-table.cjs");
class SingleStoreDialect {
  ...
  buildSelectQuery({
    ...
    const tableSql = (() => {
      ...
      if ((0, import_entity.is)(table, import_temp_table.SingleStoreTempTable)) {
        return import_sql.sql`${import_sql.sql.identifier(table.tableName)}`;
      }
      return table;
    })();
    const joinsArray = [];
    if (joins) {
      for (const [index, joinMeta] of joins.entries()) {
        ...
        } else if ((0, import_entity.is)(table2, import_temp_table.SingleStoreTempTable)) {
          const alias = joinMeta.alias !== table2.tableName ? joinMeta.alias : void 0;
          joinsArray.push(
            import_sql.sql`${import_sql.sql.raw(joinMeta.joinType)} join${lateralSql} ${import_sql.sql.identifier(table2.tableName)}${alias && import_sql.sql` ${import_sql.sql.identifier(alias)}`}${onSql}`
          );
        } else {
          joinsArray.push(
