...
import { SingleStoreTempTable } from "./temp-table.js";
  ...
  buildSelectQuery({
    ...
    if (joins) {
      for (const [index, joinMeta] of joins.entries()) {
        ...
        } else if (is(table2, SingleStoreTempTable)) {
          const alias = joinMeta.alias !== table2.tableName ? joinMeta.alias : void 0;
          joinsArray.push(
            sql`${sql.raw(joinMeta.joinType)} join${lateralSql} ${sql.identifier(table2.tableName)}${alias && sql` ${sql.identifier(alias)}`}${onSql}`
          );
        } else {
          joinsArray.push(
